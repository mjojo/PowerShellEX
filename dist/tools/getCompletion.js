 ...
 */
async function getCompletions(code, cursorPosition, maxResults = 30) {
    // Cache key: code + cursor
    const cacheKey = `${cursorPosition}:${code}`;
    const cached = tokenOptimizer_1.completionsCache.get(cacheKey);
    if (cached) {
    ...
    try {
        const result = await (0, powershell_1.runPowerShellJson)(script, undefined, 10_000, 2);
        if (!result) {
        ...
        tokenOptimizer_1.completionsCache.set(cacheKey, out);
        return out;
    }
    catch (error) {
        return {
